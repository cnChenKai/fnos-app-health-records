...
| 应用版本 | `0.1.5` | `package.json` |
| fnOS manifest 版本 | `0.1.5` | `scripts/prepare-package.mjs` 从 `package.json` 写入 |
| fnOS sub_version | `0.1.5.0` | `scripts/prepare-package.mjs` 生成 |
...
- 卸载选择“保留数据”时数据保留。
- 卸载选择“删除数据”时数据清理。
- “我的 -> 备份与恢复”可创建、下载、恢复和删除完整应用备份；恢复前会自动生成安全备份。
...
- 确认 schema 版本为目标版本。
- 检查数据目录占用显示正常。
- 进入“我的 -> 备份与恢复”，创建一份完整备份，确认文件名包含应用标识、`backup` 和时间戳，并可下载；恢复测试前保留当前数据快照。
