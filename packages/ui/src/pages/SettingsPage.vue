...
const accountRole = app.session.value?.isGatewayAdmin ? "管理员" : "家庭成员";
...
<template>
  <section class="settings-page my-page">
    ...
    <nav class="settings-menu" aria-label="设置入口">
      <RouterLink to="/me/members"><UsersRound :size="20" /><div><strong>家庭成员</strong><span>{{ app.session.value?.isGatewayAdmin ? "成员资料与访问权限" : "添加和管理自己的成员" }}</span></div><ChevronRight :size="18" /></RouterLink>
      ...
      <RouterLink v-if="app.session.value?.isGatewayAdmin" to="/me/maintenance"><Wrench :size="20" /><div><strong>维护工具</strong><span>重新生成历史 PDF 高清单页图</span></div><ChevronRight :size="18" /></RouterLink>
