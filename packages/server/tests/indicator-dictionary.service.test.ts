...
test("installs, upgrades and rolls back remote dictionaries while preserving unmatched-name history", async () => {
  ...
  try {
    ...
    assert.equal(fallbackCheck.latestRevision, 9);
    assert.match(fallbackCheck.sourceFailures.at(-1) || "", /远程 revision 4.*内置 revision 9/);
    ...
    bundle = remoteBundleUsingCoreCategory(9);
    requestedHosts.length = 0;
    ...
    assert.equal(bundledCheck.latestRevision, 9);
    ...
    assert.equal(bundledUpdate.revision, 9);
