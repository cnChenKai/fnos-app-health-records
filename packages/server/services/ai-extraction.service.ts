...
export function persistAiExtraction(reportId: string, jobId: string, result: AiExtractionResult, inputCharacters: number) {
  ...
  set("report_type", fields.reportType);
  set("report_subtype", fields.reportSubtype);
  set("hospital_name_raw", fields.hospitalNameRaw);
  set("hospital_branch", fields.hospitalBranch);
  set("city", fields.city);
  set("visit_type", fields.visitType);
  set("visit_department", fields.visitDepartment);
  set("ordering_department", fields.orderingDepartment);
  set("performing_department", fields.performingDepartment);
  set("reporting_department", fields.reportingDepartment);
  set("inpatient_ward", fields.inpatientWard);
  if (fields.bodyParts.length) set("body_parts_json", JSON.stringify(fields.bodyParts));
  if (Object.keys(fields.identifiers).length) set("identifiers_json", JSON.stringify(fields.identifiers));
  set("report_issued_at", fields.reportIssuedAt);
  set("examined_at", fields.examinedAt);
  set("ordered_at", fields.orderedAt);
  set("sampled_at", fields.sampledAt);
  set("received_at", fields.receivedAt);
  set("reviewed_at", fields.reviewedAt);
  set("admitted_at", fields.admittedAt);
  set("discharged_at", fields.dischargedAt);
  if (Object.keys(fields.clinicians).length) set("clinicians_json", JSON.stringify(fields.clinicians));
  set("clinical_diagnosis", fields.clinicalDiagnosis);
  set("purpose", fields.purpose);
  set("chief_complaint", fields.chiefComplaint);
  set("findings", fields.findings);
  set("impression", fields.impression);
  set("summary", fields.summary);
  set("recommendation", fields.recommendation);
