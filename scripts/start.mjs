...
if (!socketPath && !process.env.AUTH_MODE && process.env.NODE_ENV !== "development") {
...
if (socketPath) {
  ...
  server.listen(port, host, () => {
    ...
    const path = configuredPrefix === "/" ? "/" : `/${configuredPrefix.replace(/^\/+|\/+$/g, "")}/`;
    console.log(`Nitro listening at http://${host}:${port}${path}`);
  });
}
