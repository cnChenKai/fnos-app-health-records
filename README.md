...
当前版本已经具备核心档案闭环：概览、档案时间轴、上传处理进度、OCR 文本查看、AI 整理、报告校对、趋势、提醒、重复报告检测、回收站、完整备份恢复、运行状态、AI 配置与审计。完整功能进度见 [健康档案应用-TODO.md](./健康档案应用-TODO.md)。
...
- 数据安全：报告删除先进入 30 天回收站，可恢复或永久删除；完整备份包含数据库、原件、缩略图、配置和 AI 密钥，支持下载、恢复和删除备份记录。
...
管理员可在“我的 → 备份与恢复”创建完整应用备份。完整备份位于 `backups/full/`，格式为 `.tar.gz`，包含 SQLite 一致性快照、报告原件、分页/缩略图、运行配置和 AI 密钥；恢复前会自动额外创建一份“恢复前安全备份”。备份包包含医疗数据和密钥，请仅保存在可信设备。
