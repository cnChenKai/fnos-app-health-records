...
原件默认保存在本地 NAS 或 Docker 数据目录。升级后 OCR 仍默认使用本地方式，不会自动外发健康数据。管理员可以为之后的新批次选择 MinerU Agent 轻量解析或精准解析；这两种方式都会把处理后的完整页面副本发送至 MinerU，页面可能包含未经文本脱敏的完整健康信息，因此浏览器上传、NAS 导入、重新识别和页面调整产生的每个新批次都必须再次确认。应用只发送随机页名，不发送原始文件名或本地路径；精准解析 Token 使用 AES-256-GCM 加密保存。
